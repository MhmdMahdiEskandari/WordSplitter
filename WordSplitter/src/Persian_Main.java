	...
	public static void main(String[] args) {
		...
		try {
			in = new BufferedReader(new FileReader(
					FileSystems.getDefault().getPath("").toAbsolutePath().toString() + "/src/file/fa.words.txt"));
		...
		try {
			input = new BufferedReader(new FileReader(
					FileSystems.getDefault().getPath("").toAbsolutePath().toString() + "/src/file/mergedTokens_fa.txt"));
		} catch (FileNotFoundException e1) {
			e1.printStackTrace();
		}
		String string;
		...
		try {
			writer = new BufferedWriter(new FileWriter("94463104_Assignment1_FA"));
		} catch (IOException e) {
			e.printStackTrace();
		}
